      ...
      <div className="grid min-h-screen w-full grid-cols-1 bg-background lg:grid-cols-2">
          ...
          >
            <div className="mb-8 flex items-center gap-3">
              ...
              <div className="leading-tight">
                <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Bangalore Traffic Police</div>
                <div className="text-base font-semibold">Parking Intelligence</div>
              </div>
            </div>

            <h1 className="text-3xl font-semibold tracking-tight md:text-4xl">Welcome back</h1>
            <p className="mt-2 text-sm text-muted-foreground">Sign in to access citizen tools or the officer console.</p>

            <Tabs value={tab} onValueChange={handleTab} className="mt-8">
              <TabsList className="grid w-full grid-cols-2">
                <TabsTrigger value="user">Citizen</TabsTrigger>
                <TabsTrigger value="admin">Officer</TabsTrigger>
              </TabsList>
            </Tabs>

            <div className="mt-4 flex gap-2 text-sm">
              <button onClick={() => setMode("login")} className={"pb-0.5 " + (mode === "login" ? "border-b-2 border-[var(--saffron)] font-medium" : "text-muted-foreground")}>Sign in</button>
              <span className="text-muted-foreground">/</span>
              <button onClick={() => setMode("register")} className={"pb-0.5 " + (mode === "register" ? "border-b-2 border-[var(--saffron)] font-medium" : "text-muted-foreground")}>Register</button>
            </div>
            ...
            <form onSubmit={submit} className="mt-5 space-y-4">
              ...
              <Button type="submit" disabled={loading} className="h-11 w-full bg-[var(--saffron)] text-[var(--saffron-foreground)] hover:bg-[var(--saffron)]/90">
                {loading ? "Please wait…" : mode === "register" ? "Create account" : `Sign in ${tab === "admin" ? "as Officer" : "as Citizen"}`}
              </Button>
        ...
        <div className="relative hidden lg:col-start-2 lg:block">
          ...
          <div className="absolute bottom-10 left-8 text-white">
            ...
            <div className="mt-1 text-2xl font-semibold">Parking Intelligence</div>
            <div className="mt-2 max-w-sm text-sm text-white/80">Live enforcement analytics, challan management, and citizen services for Bengaluru.</div>
          </div>
        </div>
      </div>
    </>
