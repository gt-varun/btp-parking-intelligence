export function MapLegend({ layer }: { layer: "cli" | "count" }) {
  ...
  return (
    <div className="pointer-events-none absolute right-3 bottom-3 z-[400] rounded-lg border bg-card/95 p-3 text-xs shadow-card backdrop-blur">
      <div className="mb-1.5 font-semibold">{layer === "cli" ? "Capacity-Loss Index" : "Violation Count"}</div>
      ...
      <div className="mt-2 border-t pt-2 text-[10px] text-muted-foreground">Marker size ∝ violation volume</div>
    </div>
  );
}
